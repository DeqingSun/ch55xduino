...
void via_process(void) {
    ...
    switch( Ep2Buffer[0] ) {
        ...
        case ID_KEYMAP_GET_KEYCODE:
            {
                __data uint16_t keycode = dynamic_keymap_get_keycode(Ep2Buffer[1], Ep2Buffer[2], Ep2Buffer[3]);
        ...
        case ID_KEYMAP_GET_BUFFER:  //0x12
            {
                __data uint16_t offset = (Ep2Buffer[1] << 8) | Ep2Buffer[2];   //well I know it overflow
